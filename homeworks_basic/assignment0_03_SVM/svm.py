...
def rbf(x_1, x_2, sigma=1.):
    ...
    return torch.Tensor(torch.exp(-torch.cdist(x_1, x_2) ** 2 * (1. / (2 * sigma ** 2)))).type(torch.float32)
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = k_batch @ self.betas + self.bias  ## YOUR CODE HERE # get the matrix product using SVM
                # parameters: self.betas and self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        ...
        with torch.no_grad():
            ...
            return torch.squeeze(torch.tanh(K @ self.betas + self.bias))
